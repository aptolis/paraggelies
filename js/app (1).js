...
function checkNcReady() {
  ...
  if (name && shop && city && route) {
    btn.style.cssText = 'flex:1;padding:11px;border-radius:var(--border-radius-md);font-size:14px;font-weight:600;cursor:pointer;border:none;background:#1D9E75;color:#fff';
  } else {
    btn.style.cssText = 'flex:1;padding:11px;border-radius:var(--border-radius-md);font-size:14px;cursor:not-allowed;border:none;background:#e5e5e5;color:#aaa';
  }
}
...
function saveNewClient() {
  ...
  if (!name || !shop || !city || !routeKey) return;
  ...
  const newClient = {
    ...
    name, shop, city,
    route: routeKey,
    routeLabel: route ? route.label : routeKey,
    tel, mobile, address,
    history: {},
  };
