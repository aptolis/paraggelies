    ...
    byRoute[r.key].forEach(c => {
      ...
      html += `<div class="client-card" id="card-${c.id}" onclick="selectClient('${c.id}')">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:11px;color:var(--color-text-tertiary);margin-top:1px">Παραγγελια / Ιστορικο</div>
        ...
        <button onclick="event.stopPropagation();openEditClient('${c.id}')"
          style="border:none;background:#f0f0f0;padding:5px 10px;cursor:pointer;color:#444;border-radius:8px;font-size:11px;font-weight:600;white-space:nowrap"
          aria-label="Επεξεργασια πελατη">
          <i class="ti ti-settings" style="font-size:12px"></i> Επεξεργασια
        </button>
      </div>`;
    });
  });
  ...
  modal.innerHTML = `
    <div style="background:#fff;width:100%;border-radius:16px 16px 0 0;padding:20px;max-height:90vh;overflow-y:auto">
      ...
      <div style="margin-top:24px;display:flex;gap:10px">
        <button class="btn-cancel" onclick="document.getElementById('edit-client-modal').remove()"
          style="padding:12px 18px;font-size:15px;border-radius:var(--radius);cursor:pointer">
          <i class="ti ti-arrow-left"></i> Πισω
        </button>
        <button onclick="saveEditClient('${cid}')"
          style="flex:1;padding:12px;border-radius:var(--radius);font-size:15px;font-weight:600;cursor:pointer;border:none;background:#1D9E75;color:#fff">
          <i class="ti ti-check"></i> Αποθηκευση
        </button>
      </div>
      <button onclick="deleteClient('${cid}')"
        style="width:100%;margin-top:10px;padding:12px;border-radius:var(--radius);font-size:14px;font-weight:600;cursor:pointer;border:none;background:#FEE2E2;color:#B91C1C;display:flex;align-items:center;justify-content:center;gap:6px">
...
function deleteClient(cid) {
  const c = window.CLIENTS.find(cl => cl.id === cid);
  if (!confirm(`Διαγραφη πελατη "${c ? c.name : ''}"\n\nΘα διαγραφουν και ολα τα δεδομενα του. Συνεχεια;`)) return;
