...
let quarantine = [];
let historyMode = false; // true = επεξεργασία ιστορικού πελάτη
let npUnit = 'τεμ';
...
function checkQReady() {
  ...
  const onStyle  = 'flex:1;padding:11px;border-radius:var(--radius);font-size:13px;font-weight:600;cursor:pointer;border:none;color:#fff;';
  const offStyle = 'flex:1;padding:11px;border-radius:var(--radius);font-size:13px;cursor:not-allowed;border:none;background:#e5e5e5;color:#aaa';
...
// ============================================================
// ΝΕΟ ΠΡΟΪΟΝ — 2 επιλογές: Ιστορικό ή Εκκρεμότητα
// ============================================================
...
function addNewProd(mode) {
  ...
  // Πάντα δημιουργείται στον κατάλογο
  ...
  if (mode === 'history') {
    // → Ιστορικό: μπαίνει ξετσεκάριστο στο ιστορικό
    if (cur) {
    ...
    autoSave();
    showToast('✓ Προστέθηκε στο ιστορικό!');
  } else {
    // → Εκκρεμότητα: μπαίνει στις εκκρεμότητες του πελάτη
    if (cur) {
      if (!cur.pendingItems) cur.pendingItems = [];
      cur.pendingItems.push({ pid, name, cat, qty, unit: npUnit, addedAt: Date.now() });
    }
    autoSave();
    showToast('⏳ Προστέθηκε στις εκκρεμότητες!');
  }
...
function renderPendingItems() {
  ...
  items.forEach((item, i) => {
    ...
    const label  = isText ? item.text : item.name;
    const sub    = isText ? '' : `<div style="font-size:11px;color:#B45309">${item.cat}</div>`;
    const histBtn = isText
      ? '' // κείμενο δεν πάει στο ιστορικό
      : `<button onclick="confirmPendingItem(${i},'history')"
          style="border:none;background:#1D9E75;color:#fff;padding:4px 8px;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;white-space:nowrap">
          → Ιστορικό
        </button>`;
    html += `<div style="display:flex;align-items:center;gap:8px;padding:8px 10px;margin-bottom:6px;background:#FFFBEB;border:1px solid #FCD34D;border-radius:10px">
      ...
      ${histBtn}
      <button onclick="confirmPendingItem(${i},'skip')"
        style="border:none;background:#f0f0f0;color:#666;padding:4px 8px;border-radius:6px;font-size:11px;cursor:pointer;white-space:nowrap">
        Παράλειψη
      </button>
      <button onclick="confirmPendingItem(${i},'delete')"
        style="border:none;background:transparent;color:#ccc;padding:4px;cursor:pointer">
...
function confirmPendingItem(i, action) {
  ...
  if (action === 'history') {
    ...
    // Ενεργοποίηση στην παραγγελία αν δεν υπάρχει ήδη
    ...
    renderOrder();
    renderPendingItems();
    showToast('✓ Πέρασε στο ιστορικό!');
  } else if (action === 'skip') {
    // Παράλειψη — μένει εκκρεμής για την επόμενη φορά
    showToast('Παρέμεινε εκκρεμής');
    renderPendingItems();
...
function liveSearch(q) {
  ...
  box.innerHTML = shown.map(p => {
    ...
    return `<div class="live-result-item${already ? ' already' : ''}" style="display:flex;align-items:center;gap:8px;padding:10px 12px;border-bottom:0.5px solid var(--color-border)">
      <i class="ti ${z.icon}" style="font-size:13px;color:var(--color-text-tertiary);flex-shrink:0" aria-hidden="true"></i>
      <div style="flex:1;min-width:0;cursor:pointer" onclick="quickAdd('${p.id}')">
      ...
      <button onclick="quickAddPending('${p.id}')"
        style="border:none;background:#FEF3C7;color:#D97706;padding:4px 7px;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;flex-shrink:0"
        title="Προσθήκη στις εκκρεμότητες">
        <i class="ti ti-clock" style="font-size:12px"></i>
      </button>
      <button onclick="quickAdd('${p.id}')"
        style="border:none;background:#DCFCE7;color:#16A34A;padding:4px 7px;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;flex-shrink:0">
        ${already ? '↺' : '+'}
      </button>
    </div>`;
  }).join('');
...
function quickAddPending(pid) {
  ...
  const cat = getCatOfProd(pid);
  if (!cur.pendingItems) cur.pendingItems = [];
  // Αποφυγή διπλοεγγραφής
  if (cur.pendingItems.find(i => i.pid === pid)) {
    showToast('Ήδη στις εκκρεμότητες');
    return;
  }
  cur.pendingItems.push({ pid, name: p.name, cat, qty: 1, unit: p.unit, addedAt: Date.now() });
  autoSave();
  renderPendingItems();
  showToast(`⏳ ${p.name.substring(0,25)} → εκκρεμότητες`);
}

function quickAdd(pid) {
  if (!p) return;
  if (historyMode) {
    // Λειτουργία ιστορικού — αποθήκευση στο history χωρίς ενεργοποίηση
    const cat = getCatOfProd(pid);
    if (cat) {
      if (!cur.history) cur.history = {};
      if (!cur.history[cat]) cur.history[cat] = {};
      ...
      autoSave();
      showToast(`✓ ${p.name.substring(0,25)}... → ιστορικό`);
      // Ανανέωση live search για να φανεί το badge
...
function copyOrder() {
  ...
  if (cur && cur.pendingItems && cur.pendingItems.length) {
    txt += '=== ΕΚΚΡΕΜΟΤΗΤΕΣ (αναμένουν επιβεβαίωση) ===\n';
    cur.pendingItems.forEach(item => { txt += `• ${item.name} (${item.cat})\n`; });
    txt += '\n';
  }
  if (notesVal) {
    txt += '=== ΣΗΜΕΙΩΣΕΙΣ ===\n' + notesVal + '\n';
  ...
  navigator.clipboard.writeText(txt)
    .then(() => {
      if (cur) {
        ...
        autoSave();
        delete pendingOrders[cur.id];
        // Καθαρισμός orderState και σημειώσεων μετά αποστολή
        orderState = {};
        quarantine = [];
        const notesEl = document.getElementById('order-notes');
        if (notesEl) notesEl.value = '';
  ...
  modal.innerHTML = `
    <div style="background:#fff;width:100%;border-radius:16px 16px 0 0;padding:20px;max-height:90vh;overflow-y:auto">
      ...
      <button onclick="openHistoryMode('${cid}')"
        ...
        <i class="ti ti-history"></i> Επεξεργασία ιστορικού παραγγελιών
      </button>
    </div>`;
  document.body.appendChild(modal);
}
...
function openHistoryMode(cid) {
  // Κλείνουμε το modal αν είναι ανοιχτό
  const m = document.getElementById('edit-client-modal');
  if (m) m.remove();

  // Φορτώνουμε τον πελάτη αν δεν είναι ήδη επιλεγμένος
  if (!cur || cur.id !== cid) {
  ...
  // Πηγαίνουμε στο νέο προϊόν (live search) για γρήγορη προσθήκη
  show('s-newprod');
  setTimeout(() => {
...
function exitHistoryMode() {
  ...
  // Επαναφόρτωση ιστορικού στο orderState
  if (cur) {
    orderState = {};
    Object.entries(cur.history || {}).forEach(([cat, items]) => {
      Object.entries(items).forEach(([pid, qty]) => {
