...
function filterClients(q) {
  ...
  ROUTES.forEach(r => {
    ...
    byRoute[r.key].forEach(c => {
      ...
      const inPrintQueue = pq.some(o => o.clientId === c.id);
      const cardExtra = inPrintQueue ? ' style="border:2px solid #1D9E75;background:#F0FDF4"' : '';
      html += `<div class="client-card" id="card-${c.id}" onclick="selectClient('${c.id}')"${cardExtra}>`;
    });
  });
  ...
  window._printHTML = `<!DOCTYPE html><html lang="el"><head>
    ...
    <style>
      ...
      * { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
      /* Κρυβει browser header/footer οπου υποστηριζεται */
      @page { margin-top: 15mm; margin-bottom: 8mm; }
      html { margin: 0; }
    </style>
  </head><body>
