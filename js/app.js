...
function openNewProdFromCatalog() {
  npFromCatalog = true;
  _openNewProdCommon();
  // Catalog mode: δειχνει μονο "Αποθηκευση", κρυβει Εκκρεμοτητα/Ιστορικο
  document.getElementById('btn-q-catalog').style.display  = 'flex';
  document.getElementById('btn-q-pending').style.display  = 'none';
  document.getElementById('btn-q-history').style.display  = 'none';
  show('s-newprod');
}
...
function openNewProd() {
  npFromCatalog = false;
  _openNewProdCommon();
  // Order mode: δειχνει Εκκρεμοτητα/Ιστορικο, κρυβει Αποθηκευση
  document.getElementById('btn-q-catalog').style.display  = 'none';
  document.getElementById('btn-q-pending').style.display  = 'flex';
  document.getElementById('btn-q-history').style.display  = 'flex';
  show('s-newprod');
}
...
function checkQReady() {
  ...
  const onStyle  = 'flex:1;padding:11px;border-radius:var(--radius);font-size:14px;font-weight:600;cursor:pointer;border:none;color:#fff;pointer-events:auto;display:flex;align-items:center;justify-content:center;gap:6px;';
  const offStyle = 'flex:1;padding:11px;border-radius:var(--radius);font-size:14px;cursor:not-allowed;border:none;background:#e5e5e5;color:#aaa;pointer-events:none;display:flex;align-items:center;justify-content:center;gap:6px;';
  ...
  if (bc) bc.style.cssText = ready ? onStyle + 'background:#1D9E75' : offStyle;
...
function addNewProd(mode) {
  ...
  if (mode === 'history') {
    ...
    // Απλη αποθηκευση στον καταλογο χωρις παραγγελια
    autoSave();
    ...
    if (!clients.length) {
      ...
      Object.entries(byRoute).forEach(([rk, rClients]) => {
        ...
        rClients.forEach(c => {
          ...
          html += `<div style="display:flex;align-items:center;gap:8px;padding:5px 0;border-bottom:0.5px solid #f0f0f0">`;
          html += `<div style="flex:1;font-size:13px;color:${done ? '#aaa' : '#1a1a1a'};${done ? 'text-decoration:line-through' : ''}">`;
          html += `${c.name}${timeStr}</div>`;
          if (done) {
            html += `<span style="font-size:11px;font-weight:600;color:#1D9E75;white-space:nowrap"><i class="ti ti-check" style="font-size:10px" aria-hidden="true"></i> Ολοκληρωθηκε</span>`;
          }
          html += `</div>`;
        });
      });
