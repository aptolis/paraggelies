...
function openPrintPreview() {
  ...
  let cols = q.map(order => {
    ...
    Object.entries(order.zones).forEach(([zk, zone]) => {
      ...
      html += `<div class="print-zone${isSyn ? ' syn-zone' : ''}"${isSyn ? ' style="background:#c0c0c0;color:#000;"' : ''}>${zLabel}</div>`;
      Object.entries(zone.cats).forEach(([cat, items]) => {
        const total = buildCatTotalsText(items);
        html += `<div class="print-cat${isSyn ? ' syn-bg' : ''}"${isSyn ? ' style="background:#d8d8d8;"' : ''}><span>${cat}</span><span class="print-cat-total">${total}</span></div>`;
        items.forEach(item => {
          html += `<div class="print-item${isSyn ? ' syn-bg' : ''}"${isSyn ? ' style="background:#d8d8d8;"' : ''}><span class="print-qty">${item.qty} ${item.unit}</span> ${item.name}</div>`;
        });
      });
    });
