...
function openPrintPreview() {
  ...
  let cols = q.map(order => {
    ...
    Object.entries(order.zones).forEach(([zk, zone]) => {
      ...
      html += `<div class="print-zone" style="${zStyle}">${zLabel}</div>`;
    ...
    if (order.pendingItems && order.pendingItems.length) {
      html += `<div class="print-zone" style="background:#fff8e1;border-top:1.5px solid #FCD34D;border-bottom:1.5px solid #FCD34D;">ΕΚΚΡΕΜΟΤΗΤΕΣ</div>`;
      order.pendingItems.forEach(item => {
        if (item.type === 'text') {
          html += `<div class="print-item"><span class="print-qty">—</span> ${item.text}</div>`;
        } else {
          html += `<div class="print-item"><span class="print-qty">${item.qty || 1} ${item.unit || ''}</span> ${item.name}${item.note ? ' (' + item.note + ')' : ''}</div>`;
        }
      });
    }
    html += `</div>`;
  ...
  window._printHTML = `<!DOCTYPE html><html lang="el"><head>
    ...
    <style>
      * { margin:0; padding:0; box-sizing:border-box; }
      ...
      .print-header { text-align:center; font-size:13px; font-weight:700; margin-bottom:8px; border-bottom:2px solid #000; padding-bottom:5px; }
      .print-grid { columns:${numCols}; column-gap:8px; }
      .print-col { break-inside:avoid; border:1px solid #bbb; border-radius:3px; padding:5px; margin-bottom:8px; display:inline-block; width:100%; }
      .print-client { font-size:12px; font-weight:700; border-bottom:1.5px solid #000; padding-bottom:2px; margin-bottom:3px; }
      .print-shop { font-size:8px; color:#555; margin-bottom:4px; }
      .print-zone { font-size:9px; font-weight:700; text-transform:uppercase; padding:2px 5px; margin:4px -5px 2px -5px; letter-spacing:0.5px; background:#eee; color:#000; }
      .print-cat { font-size:9px; font-weight:600; color:#333; margin:2px 0 1px 0; border-bottom:0.5px solid #e0e0e0; display:flex; justify-content:space-between; gap:4px; }
      ...
      .print-qty { font-weight:700; white-space:nowrap; min-width:26px; }
      @page { size:A4; margin:20mm 8mm 10mm 8mm; }
    </style>
  </head><body>
