...
function addNewProd(mode) {
  ...
  // Παντα δημιουργειται στον καταλογο
  ...
  if (mode === 'history') {
    ...
    // → Εκκρεμοτητα: μπαινει στις εκκρεμοτητες του πελατη
    if (cur) {
...
function filterClients(q) {
  ...
  ROUTES.forEach(r => {
    ...
    byRoute[r.key].forEach(c => {
      ...
      html += `<div class="client-card" id="card-${c.id}" onclick="selectClient('${c.id}')">
        <div class="avatar av-${r.cls}">${initials}</div>
        <div style="flex:1;min-width:0">
          <div class="client-name">
            ${c.name}
            ${hasPending ? '<span class="pending-badge"><i class="ti ti-clock" aria-hidden="true"></i> Εκκρεμης</span>' : ''}
          </div>
    ...
    if (isText) {
      ...
      html += `<div style="padding:10px;margin-bottom:8px;background:#FFFBEB;border:1px solid #FCD34D;border-radius:10px">
        ...
        <textarea id="pnote-${i}" placeholder="Σημειωση για αυτο το προιον..."
          onchange="savePendingNote(${i},this.value)"
  ...
  Object.entries(window.PRODUCTS).forEach(([cat, prods]) => {
    ...
    filtered.forEach(p => {
      ...
      html += `<div class="catalog-prow${sel ? ' selected' : ''}" onclick="toggleCatSel('${p.id}')">
        ...
        ${isCustom ? `<button onclick="event.stopPropagation();openEditProd('${p.id}')" style="border:none;background:transparent;padding:4px 6px;cursor:pointer;color:var(--color-text-tertiary)" aria-label="Επεξεργασια"><i class="ti ti-pencil" style="font-size:14px"></i></button>` : ''}
      </div>`;
    });
    html += '</div>';
  });
...
function addNewProd() {
  ...
  // Αυτοματη ενεργοποιηση στην παραγγελια
...
function confirmDeleteProd(pid) {
  const p = getProd(pid);
  if (!confirm(`Διαγραφη "${p ? p.name : pid}" απο τον καταλογο;`)) return;
  const cat = getCatOfProd(pid);
  ...
  modal.innerHTML = `
    <div style="background:#fff;width:100%;border-radius:16px 16px 0 0;padding:20px;max-height:90vh;overflow-y:auto">
      ...
      <div class="modal-acts" style="margin-top:20px">
        ...
        <button class="btn-cancel" onclick="document.getElementById('edit-client-modal').remove()">Ακυρωση</button>
      ...
      <button onclick="openHistoryMode('${cid}')"
        style="width:100%;margin-top:10px;padding:11px;border-radius:var(--radius);font-size:13px;font-weight:600;cursor:pointer;border:1px dashed #1D9E75;background:transparent;color:#1D9E75;display:flex;align-items:center;justify-content:center;gap:6px">
        <i class="ti ti-history"></i> Επεξεργασια ιστορικου παραγγελιων
      </button>
      <button onclick="openClearHistoryModal('${cid}')"
        style="width:100%;margin-top:8px;padding:11px;border-radius:var(--radius);font-size:13px;font-weight:600;cursor:pointer;border:1px dashed #DC2626;background:transparent;color:#DC2626;display:flex;align-items:center;justify-content:center;gap:6px">
        <i class="ti ti-trash"></i> Διαχειριση / Καθαρισμος ιστορικου
      </button>
    </div>`;
  document.body.appendChild(modal);
}
...
function deleteClient(cid) {
  const c = window.CLIENTS.find(cl => cl.id === cid);
  if (!confirm(`Διαγραφη πελατη "${c ? c.name : ''}";\nΘα διαγραφει και το ιστορικο παραγγελιων του.`)) return;
...
function removeOneFromHistory(cid, pid, btn) {
  if (!confirm('Αφαιρεση αυτου του προιοντος απο το ιστορικο;')) return;
  const c = window.CLIENTS.find(cl => cl.id === cid);
  if (!c) return;
...
function clearAllHistory(cid) {
  if (!confirm('Διαγραφη ΟΛΟΥ του ιστορικου για αυτον τον πελατη;\nΑυτη η ενεργεια δεν αναιρειται!')) return;
  const c = window.CLIENTS.find(cl => cl.id === cid);
  if (!c) return;
  c.history = {};
  autoSave();
  document.getElementById('clear-history-modal').remove();
...
function openHistoryMode(cid) {
  ...
  if (banner) {
    ...
    document.querySelector('.app').style.paddingTop = '40px';
  }
...
function exitHistoryMode() {
  ...
  // Επαναφορτωση ιστορικου στο orderState
  if (cur) {
    orderState = {};
    Object.entries(cur.history || {}).forEach(([cat, items]) => {
      Object.entries(items).forEach(([pid, qty]) => {
...
function saveForPrint() {
  ...
  updatePrintBanner();
  showToast('Αποθηκευτηκε για εκτυπωση! ✓');
  show('s-clients');
}
...
function openPrintPreview() {
  ...
  let cols = q.map(order => {
    ...
    Object.entries(order.zones).forEach(([zk, zone]) => {
      html += `<div class="print-zone">${zone.label}</div>`;
      Object.entries(zone.cats).forEach(([cat, items]) => {
        const total = buildCatTotalsText(items);
        html += `<div class="print-cat"><span>${cat}</span><span class="print-cat-total">${total}</span></div>`;
        items.forEach(item => {
          html += `<div class="print-item"><span class="print-qty">${item.qty} ${item.unit}</span> ${item.name}</div>`;
        });
      });
    });
    html += `<div class="print-time">${order.date} ${order.time}</div></div>`;
    return html;
  }).join('');

  const dateStr = new Date().toLocaleDateString('el-GR', { weekday:'long', day:'numeric', month:'long', year:'numeric' });

  // Αποθηκευση HTML για εκτυπωση στο iframe
  window._printHTML = `<!DOCTYPE html><html lang="el"><head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Παραγγελιες ${dateStr}</title>
    <style>
      * { margin:0; padding:0; box-sizing:border-box; }
      body { font-family: Arial, sans-serif; font-size: 12px; padding: 8mm 8mm 8mm 8mm; }
      .print-header { text-align:center; font-size:14px; font-weight:700; margin-bottom:10px; border-bottom:2px solid #000; padding-bottom:6px; }
      .print-grid { display:flex; flex-wrap:wrap; gap:8px; align-items:flex-start; }
      .print-col { flex:1; min-width:120px; border:1px solid #ccc; border-radius:4px; padding:6px; page-break-inside:avoid; break-inside:avoid; }
      .print-client { font-size:13px; font-weight:700; border-bottom:1px solid #000; padding-bottom:3px; margin-bottom:4px; }
      .print-shop { font-size:9px; color:#555; margin-bottom:6px; }
      .print-zone { font-size:9px; font-weight:700; text-transform:uppercase; background:#eee; padding:2px 4px; margin:5px -6px 3px -6px; letter-spacing:0.5px; }
      .print-cat { font-size:9px; font-weight:600; color:#333; margin:3px 0 1px 0; border-bottom:0.5px solid #ddd; display:flex; justify-content:space-between; gap:4px; }
      .print-cat-total { font-weight:700; color:#1a7a55; white-space:nowrap; }
      .print-item { font-size:10px; padding:1px 0 1px 4px; display:flex; gap:4px; }
      .print-qty { font-weight:700; white-space:nowrap; min-width:28px; }
      .print-time { font-size:8px; color:#999; margin-top:8px; text-align:right; }
      @page { size: A4; margin-top:20mm; margin-bottom:10mm; }
    </style>
  </head><body>
    <div class="print-header">ΠΑΡΑΓΓΕΛΙΕΣ — ${dateStr}</div>
    <div class="print-grid">${cols}</div>
  </body></html>`;

  // Προεπισκοπηση μεσα στην εφαρμογη
  const previewHTML = `
...
function doPrint() {
  const html = window._printHTML;
  if (!html) { showToast('Δεν υπαρχει περιεχομενο', 'error'); return; }

  // Αφαιρουμε παλιο iframe αν υπαρχει
...
function clearPrintQueue() {
  if (!confirm('Διαγραφη ολων των παραγγελιων απο την ουρα εκτυπωσης;\nΚανε αυτο ΜΟΝΟ αφου εχεις εκτυπωσει/στειλει!')) return;
  savePrintQueue([]);
  updatePrintBanner();
  show('s-clients');
