    ...
    showP.forEach(p => {
      ...
      zHTML[zk] += `
        <div class="prow" style="${rowStyle}" id="row-${p.id}">
          ...
          ${inH ? `<button onclick="removeFromHistory('${p.id}')" style="border:none;background:transparent;padding:4px 5px;cursor:pointer;color:#ccc;margin-left:2px" aria-label="Αφαιρεση απο ιστορικο" title="Αφαιρεση απο ιστορικο"><i class="ti ti-x" style="font-size:13px"></i></button>` : ''}
        </div>`;
    });
  });
...
function openPrintPreview() {
  ...
  let cols = q.map(order => {
    let html = `<div class="print-col">
      ...
      <div class="print-shop">${order.shop} &mdash; ${order.routeLabel}</div>`;
    Object.entries(order.zones).forEach(([zk, zone]) => {
      ...
      Object.entries(zone.cats).forEach(([cat, items]) => {
        html += `<div class="print-cat">${cat}</div>`;
        items.forEach(item => {
  ...
  win.document.write(`<!DOCTYPE html><html lang="el"><head>
    <meta charset="UTF-8">
    <title>Εκτυπωση Παραγγελιων</title>
    <style>
      * { margin:0; padding:0; box-sizing:border-box; }
      body { font-family: Arial, sans-serif; font-size: 11px; padding: 10px; }
      .print-header { text-align:center; font-size:13px; font-weight:700; margin-bottom:10px; border-bottom:2px solid #000; padding-bottom:6px; }
      .print-grid { display:flex; gap:8px; align-items:flex-start; }
      .print-col { flex:1; border:1px solid #ccc; border-radius:4px; padding:6px; min-width:0; page-break-inside:avoid; break-inside:avoid; }
      .print-client { font-size:12px; font-weight:700; border-bottom:1px solid #000; padding-bottom:3px; margin-bottom:4px; }
      ...
      .print-cat { font-size:9px; font-weight:600; color:#333; margin:3px 0 1px 0; border-bottom:0.5px solid #ddd; }
      .print-item { font-size:10px; padding:1px 0; display:flex; gap:4px; }
      .print-qty { font-weight:700; white-space:nowrap; min-width:30px; }
      .print-time { font-size:8px; color:#999; margin-top:8px; text-align:right; }
      .print-footer { position:fixed; bottom:6px; right:10px; font-size:8px; color:#999; }
      @media print {
        body { padding-top:20px; }
        button { display:none !important; }
        .print-footer { display:block; }
        @page { margin-top:18mm; margin-bottom:10mm; @bottom-right { content: "Σελ. " counter(page) " / " counter(pages); font-size:8pt; color:#999; } }
      }
    </style>
  </head><body>
    <div class="print-header">ΠΑΡΑΓΓΕΛΙΕΣ &mdash; ${new Date().toLocaleDateString('el-GR', { weekday:'long', day:'numeric', month:'long', year:'numeric' })}</div>
    <div class="print-grid">${cols}</div>
    <div style="margin-top:14px;text-align:center">
      <button onclick="window.print();setTimeout(()=>{window.close()},500)" style="padding:10px 30px;font-size:14px;font-weight:700;background:#1D9E75;color:#fff;border:none;border-radius:8px;cursor:pointer">
        Εκτυπωση / Αποθηκευση PDF
      </button>
      <button onclick="window.close()" style="padding:10px 20px;font-size:14px;margin-left:10px;background:#eee;border:none;border-radius:8px;cursor:pointer">
        Κλεισιμο χωρις εκτυπωση
      </button>
    </div>
  </body></html>`);
  win.document.close();

  // Καθαρισμος ουρας οταν κλεισει το παραθυρο
  win.onbeforeunload = () => {
    savePrintQueue([]);
    updatePrintBanner();
  };
