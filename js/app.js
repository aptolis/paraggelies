...
function openPrintPreview() {
  ...
  let cols = q.map(order => {
    ...
    Object.entries(order.zones).forEach(([zk, zone]) => {
      ...
      // Για συντηρηση: σκουρο label + γκρι φοντο σε ολα τα περιεχομενα
      html += `<div class="print-zone${isSyn ? ' syn-zone' : ''}">${zLabel}</div>`;
      Object.entries(zone.cats).forEach(([cat, items]) => {
        const total = buildCatTotalsText(items);
        html += `<div class="print-cat${isSyn ? ' syn-bg' : ''}"><span>${cat}</span><span class="print-cat-total">${total}</span></div>`;
        items.forEach(item => {
          html += `<div class="print-item${isSyn ? ' syn-bg' : ''}"><span class="print-qty">${item.qty} ${item.unit}</span> ${item.name}</div>`;
        });
      });
    });
  ...
  window._printHTML = `<!DOCTYPE html><html lang="el"><head>
    ...
    <style>
      * { margin:0; padding:0; box-sizing:border-box; }
      ...
      .print-zone { font-size:9px; font-weight:700; text-transform:uppercase; padding:2px 5px; margin:4px -5px 2px -5px; letter-spacing:0.5px; background:#ddd; color:#000; }
      .syn-zone { background:#555; color:#fff; }
      .syn-bg { background:#e8e8e8; }
      .pend-zone { background:#bbb; color:#000; }
      .print-cat { font-size:9px; font-weight:600; color:#000; margin:2px 0 1px 0; border-bottom:0.5px solid #ccc; display:flex; justify-content:space-between; gap:4px; }
