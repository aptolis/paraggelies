...
let npUnit = 'τεμ';
let npFromCatalog = false; // true αν ανοιχτηκε απο τον καταλογο
...
function _openNewProdCommon() {
  document.getElementById('np-search').value = '';
  document.getElementById('np-zone').value = '';
  document.getElementById('live-results').style.display = 'none';
  document.getElementById('np-cat').value = '';
  document.getElementById('np-cat').disabled = true;
  document.getElementById('np-cat').innerHTML = '<option value="">Επιλεξε πρωτα ζωνη...</option>';
  document.getElementById('np-cat').style.color = '#aaa';
...
function openNewProd() {
  npFromCatalog = false;
  _openNewProdCommon();
  // Order mode: Εκκρεμοτητα + Ιστορικο, οχι Αποθηκευση
  const bc = document.getElementById('btn-q-catalog');
...
function openNewProdFromCatalog() {
  npFromCatalog = true;
  _openNewProdCommon();
  // Catalog mode: μονο Αποθηκευση
  const bc = document.getElementById('btn-q-catalog');
  const bp = document.getElementById('btn-q-pending');
  const bh = document.getElementById('btn-q-history');
...
function newProdGoBack() {
  if (npFromCatalog) show('s-catalog');
  else show('s-order');
}
...
function onNpZoneChange() {
  const zoneKey = document.getElementById('np-zone').value;
  const catSel  = document.getElementById('np-cat');
  catSel.value  = '';
  if (!zoneKey || !ZONES || !ZONES[zoneKey]) {
    catSel.disabled = true;
    catSel.style.color = '#aaa';
    catSel.innerHTML = '<option value="">Επιλεξε πρωτα ζωνη...</option>';
  } else {
    const cats = ZONES[zoneKey].cats;
    catSel.disabled = false;
    catSel.style.color = '';
    catSel.innerHTML = '<option value="">Επιλεξε κατηγορια...</option>' +
...
function checkQReady() {
  ...
  const ready = !!(name && cat && zone);
  const onBase  = 'flex:1;padding:11px;border-radius:var(--radius);font-size:14px;font-weight:600;cursor:pointer;border:none;color:#fff;pointer-events:auto;';
  const offBase = 'flex:1;padding:11px;border-radius:var(--radius);font-size:14px;cursor:not-allowed;border:none;background:#e5e5e5;color:#aaa;pointer-events:none;';
  [
    { id: 'btn-q-catalog', color: '#1D9E75' },
    { id: 'btn-q-pending', color: '#EF9F27' },
    { id: 'btn-q-history', color: '#1D9E75' },
  ].forEach(({ id, color }) => {
    const btn = document.getElementById(id);
    if (!btn) return;
    const disp = btn.style.display; // διατηρει το display (none/block)
    btn.style.cssText = ready ? onBase + `background:${color}` : offBase;
...
function addNewProd(mode) {
  ...
  if (mode === 'history') {
    ...
    autoSave();
    showToast('✓ Προστεθηκε στον καταλογο!');
  } else {
    if (cur) {
...
let catalogOrigin = 's-order'; // απο που ανοιχτηκε ο καταλογος

function openCatalogFromHome() {
  catalogOrigin = 's-clients';
  renderCatalog('');
  show('s-catalog');
}

function openCatalogFromOrder() {
  catalogOrigin = 's-order';
  show('s-catalog');
}
...
function filterClients(q) {
  ...
  ROUTES.forEach(r => {
    ...
    byRoute[r.key].forEach(c => {
      ...
      const schedToday = isClientScheduledToday(c);
      const schedDone  = isClientDoneToday(c);
      const schedBadge = schedDone
        ? ' <span style="display:inline-flex;align-items:center;gap:3px;background:#E1F5EE;color:#0F6E56;border-radius:6px;padding:2px 7px;font-size:10px;font-weight:600;vertical-align:middle"><i class="ti ti-check" style="font-size:9px"></i> Ολοκληρωθηκε</span>'
        : schedToday
        ? (() => { const t = c.schedule && c.schedule.timeFrom ? ' ' + c.schedule.timeFrom : ''; return ` <span style="display:inline-flex;align-items:center;gap:3px;background:#FAEEDA;color:#633806;border-radius:6px;padding:2px 7px;font-size:10px;font-weight:600;vertical-align:middle"><i class="ti ti-calendar-event" style="font-size:9px"></i> Σημερα${t}</span>`; })()
        : '';
      html += `<div class="client-card" id="card-${c.id}" onclick="selectClient('${c.id}')"${inQueue ? ' style="border:1.5px solid var(--green);background:var(--green-light)"' : ''}>
        ...
        <div style="flex:1;min-width:0">
          <div class="client-name">${c.name}${hasPending ? ' <span class="pending-badge"><i class="ti ti-clock" style="font-size:9px"></i> Εκκρεμης</span>' : ''}${schedBadge}</div>
          ${c.shop ? `<div class="client-meta"><i class="ti ti-building-store" style="font-size:11px"></i> ${c.shop}</div>` : ''}
          ${c.lastOrderAt ? `<div class="client-meta" style="color:var(--green);font-size:16px;font-weight:500"><i class="ti ti-clock" style="font-size:14px"></i> Τελ. παραγγελια ${new Date(c.lastOrderAt).toLocaleDateString('el-GR',{weekday:'short',day:'2-digit',month:'2-digit',year:'2-digit'})} ${new Date(c.lastOrderAt).toLocaleTimeString('el-GR',{hour:'2-digit',minute:'2-digit'})}</div>` : ''}
        </div>
...
function selectClient(id) {
  // Αν εκκρεμει επιβεβαιωση εκτυπωσης, μπλοκαρισμα
  if (isPrintPendingConfirm()) {
    _showPrintBlockModal();
    return;
  }
  ...
  // Σημειωσεις πελατη — εμφανιση banner αν υπαρχουν
  let notesBanner = document.getElementById('client-notes-banner');
  if (!notesBanner) {
    notesBanner = document.createElement('div');
    notesBanner.id = 'client-notes-banner';
    const voiceHint = document.getElementById('voice-hint');
    if (voiceHint) voiceHint.parentNode.insertBefore(notesBanner, voiceHint.nextSibling);
  }
  if (cur.clientNotes) {
    notesBanner.style.cssText = 'display:block;background:#EFF6FF;border-bottom:0.5px solid #BFDBFE;padding:7px 14px;font-size:12px;color:#1e40af';
    notesBanner.innerHTML = `<i class="ti ti-notes" style="font-size:12px" aria-hidden="true"></i> <strong>Σημ.:</strong> ${cur.clientNotes}`;
  } else {
...
function goSummary() {
  ...
  // Flush text input αν ο χρηστης εχει γραψει κατι αλλα δεν εχει πατησει +
  const textInp = document.getElementById('pending-text-input');
  if (textInp && textInp.value.trim() && cur) {
    if (!cur.pendingItems) cur.pendingItems = [];
    cur.pendingItems.push({ type: 'text', text: textInp.value.trim(), addedAt: Date.now() });
    textInp.value = '';
    autoSave();
  }

  // Προειδοποιηση για προιοντα με ποσοτητα αλλα ΟΧΙ τσεκαρισμενα — επιστροφη για διορθωση
  ...
  if (hasQtyNotChecked.length) {
    ...
    // Highlight τα προιοντα που δεν ειναι τσεκαρισμενα
    hasQtyNotChecked.forEach(([pid]) => {
      const row = document.getElementById('row-' + pid);
      if (row) {
        const orig = row.style.background;
        row.style.background = '#FFF3CD';
        row.style.outline = '2px solid #D97706';
        setTimeout(() => { row.style.background = orig; row.style.outline = ''; }, 3000);
      }
    });
    ...
    m.innerHTML = `
      <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;box-shadow:0 8px 32px rgba(0,0,0,0.18)">
        <div style="font-size:15px;color:#222;margin-bottom:20px;line-height:1.5">
          ⚠️ <strong>${hasQtyNotChecked.length} προιον(τα)</strong> εχουν ποσοτητα αλλα ΔΕΝ ειναι τσεκαρισμενα:<br><br>${names}${more}<br><br>Τσεκαρε τα ή μηδενισε την ποσοτητα τους.
        </div>
        <button onclick="document.getElementById('custom-confirm-modal').remove()"
          style="width:100%;padding:12px;border-radius:10px;border:none;background:#1D9E75;color:#fff;font-size:14px;font-weight:600;cursor:pointer">
          ← Πισω για διορθωση
        </button>
      </div>`;
...
function openEditClient(cid) {
  ...
  modal.innerHTML = `
    <div style="background:#fff;width:100%;border-radius:16px 16px 0 0;padding:20px;max-height:90vh;overflow-y:auto">
      ...
      <div class="field-label" style="margin-top:14px">Σημειωσεις πελατη</div>
      <textarea class="field-input" id="ec-clientnotes" placeholder="π.χ. ωρες παραλαβης, ιδιαιτεροτητες, χρεη..."
        style="min-height:80px;resize:vertical;font-family:inherit;font-size:14px">${c.clientNotes || ''}</textarea>
      ${buildScheduleHTML(c)}
...
function saveEditClient(cid) {
  ...
  c.clientNotes = document.getElementById('ec-clientnotes').value.trim();
  // Αποθηκευση προγραμματος — διατηρουμε το doneOn
  const newSched = readScheduleFromModal();
  if (c.schedule && c.schedule.doneOn) newSched.doneOn = c.schedule.doneOn;
  c.schedule = newSched;
  autoSave();
...
function removeOneFromHistory(cid, pid, btn) {
  const prodName = getProd(pid)?.name || pid;
  customConfirm(
    `Αφαιρεση απο ιστορικο;<br><br><strong style="color:#1a1a1a">${prodName}</strong>`,
    () => {
      const c = window.CLIENTS.find(cl => cl.id === cid);
      if (!c) return;
...
function saveForPrint() {
  ...
  // Flush text input αν ο χρηστης εχει γραψει κατι αλλα δεν εχει πατησει +
  const textInpPrint = document.getElementById('pending-text-input');
  if (textInpPrint && textInpPrint.value.trim() && cur) {
    if (!cur.pendingItems) cur.pendingItems = [];
    cur.pendingItems.push({ type: 'text', text: textInpPrint.value.trim(), addedAt: Date.now() });
    textInpPrint.value = '';
    autoSave();
  }
...
function openPrintPreview() {
  ...
  let cols = q.map(order => {
    ...
    Object.entries(order.zones).forEach(([zk, zone]) => {
      ...
      Object.entries(zone.cats).forEach(([cat, items]) => {
        ...
        const dispCat = cat.replace(/ \/ ΣΥΝΤΗΡΗΣΗ$/, '');
        html += `<div class="print-cat" ${itemStyle}><span>${dispCat}</span><span class="print-cat-total">${total}</span></div>`;
        const isAllantika = cat === 'ΑΛΛΑΝΤΙΚΑ / ΣΥΝΤΗΡΗΣΗ';
        const isTyria    = cat === 'ΤΥΡΙΑ / ΣΥΝΤΗΡΗΣΗ';
        items.forEach(item => {
          html += `<div class="print-item" ${itemStyle}><span class="print-qty">${item.qty} ${item.unit}</span> ${item.name}</div>`;
          const needsWeight = isAllantika ||
            (isTyria && /μπαστουνι|δοχειο/i.test(item.name));
          if (needsWeight) {
            html += `<div class="print-item-spacer"></div>`;
          }
        });
      });
    });
  ...
  window._printHTML = `<!DOCTYPE html><html lang="el"><head>
    ...
    <style>
      * { margin:0; padding:0; box-sizing:border-box; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
      ...
      .print-item-spacer { height:12px; border-bottom:0.5px solid #ccc; margin:1px 0 2px 6px; }
  ...
  const previewCols = q.map((order, idx) => {
    return `<div style="position:relative;margin-bottom:8px">
      <button onclick="removeFromPrintQueue(${idx})"
        style="position:absolute;top:4px;right:4px;z-index:2;background:#DC2626;color:#fff;border:none;border-radius:5px;padding:2px 7px;font-size:10px;font-weight:700;cursor:pointer;line-height:1.6">
        ✕ Ακυρωση
      </button>
      <div style="border:1px solid #ddd;border-radius:6px;padding:8px;font-size:11px">
        <div style="font-weight:700;font-size:12px;border-bottom:1px solid #ccc;padding-bottom:3px;margin-bottom:4px;padding-right:60px">${order.clientName}</div>
        <div style="color:#666;font-size:10px;margin-bottom:4px">${order.shop} — ${order.routeLabel}</div>
        ${Object.entries(order.zones).map(([zk, zone]) => {
          const zLabel = zk === 'kty' ? 'ΚΑΤΑΨΥΞΗ' : zone.label.toUpperCase();
          return `<div style="background:#ddd;font-size:9px;font-weight:700;padding:1px 4px;margin:4px -8px 2px -8px;text-transform:uppercase">${zLabel}</div>` +
            Object.entries(zone.cats).map(([cat, items]) => {
              const dispCat = cat.replace(/ \/ ΣΥΝΤΗΡΗΣΗ$/, '');
              return `<div style="font-size:9px;font-weight:600;border-bottom:0.5px solid #eee;margin:2px 0 1px">${dispCat}</div>` +
                items.map(item => `<div style="font-size:10px;padding-left:6px">${item.qty} ${item.unit} ${item.name}</div>`).join('');
            }).join('');
        }).join('')}
      </div>
    </div>`;
...
function removeFromPrintQueue(idx) {
  ...
  const name = q[idx].clientName;
  customConfirm(`Ακυρωση παραγγελιας ${name};`, () => {
    const q2 = getPrintQueue();
    q2.splice(idx, 1);
    savePrintQueue(q2);
    updatePrintBanner();
    filterClients('');
    if (q2.length === 0) {
      show('s-clients');
      showToast('Ουρα κενη — ολες οι παραγγελιες ακυρωθηκαν');
    } else {
      openPrintPreview();
      showToast(`Παραγγελια "${name}" ακυρωθηκε ✓`);
    }
  }, 'Ακυρωση', '#D97706');
}
...
function doPrint() {
  ...
  setTimeout(() => {
    ...
    markScheduleDoneFromPrint();
    // Ενημερωση lastOrderAt για ολους τους πελατες της ουρας
    const printedAt = Date.now();
    getPrintQueue().forEach(order => {
      const c = window.CLIENTS.find(x => x.id === order.clientId);
      if (c) c.lastOrderAt = printedAt;
    });
    autoSave();
    // ΔΕΝ καθαριζουμε αυτοματα — ο χρηστης επιβεβαιωνει χειροκινητα
    window._printHTML = null;
    // Εμφανισε banner επιβεβαιωσης
    _showPrintConfirmBanner();
  }, 300);
}

function _showPrintConfirmBanner() {
  // Αποθηκευε στο localStorage οτι η ουρα αναμενει επιβεβαιωση
  localStorage.setItem('orderapp_print_pending_confirm', '1');
  _renderPrintConfirmBanner();
}

function _renderPrintConfirmBanner() {
  const needsConfirm = localStorage.getItem('orderapp_print_pending_confirm') === '1';
  const banner = document.getElementById('print-confirm-banner');
  if (!banner) return;
  if (needsConfirm) {
    banner.style.display = 'block';
  } else {
...
function confirmPrintSent() {
  // Ο χρηστης επιβεβαιωνει — καθαριζουμε ουρα
  savePrintQueue([]);
  localStorage.removeItem('orderapp_print_pending_confirm');
  updatePrintBanner();
  filterClients('');
  _renderPrintConfirmBanner();
  show('s-clients');
  showToast('Ουρα καθαριστηκε ✓ Ετοιμος για επομενη παραγγελια!');
}

function isPrintPendingConfirm() {
  return localStorage.getItem('orderapp_print_pending_confirm') === '1';
}

function _showPrintBlockModal() {
  const old = document.getElementById('custom-confirm-modal');
  if (old) old.remove();
  const m = document.createElement('div');
  m.id = 'custom-confirm-modal';
  m.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:2000;display:flex;align-items:center;justify-content:center;padding:20px';
  m.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;box-shadow:0 8px 32px rgba(0,0,0,0.22)">
      <div style="font-size:22px;text-align:center;margin-bottom:10px">🖨️</div>
      <div style="font-size:15px;font-weight:700;color:#1a1a1a;margin-bottom:10px;text-align:center">Εκκρεμει επιβεβαιωση αποστολης</div>
      <div style="font-size:13px;color:#555;margin-bottom:20px;line-height:1.55;text-align:center">
        Επιβεβαιωσε πρωτα οτι η εκτυπωση / αποστολη PDF ολοκληρωθηκε και καθαρισε την ουρα για να συνεχισεις.
      </div>
      <button onclick="document.getElementById('custom-confirm-modal').remove();openPrintPreview()"
        style="width:100%;padding:12px;border-radius:10px;border:none;background:#1D9E75;color:#fff;font-size:14px;font-weight:700;cursor:pointer;margin-bottom:8px">
        <i class="ti ti-printer"></i> Προβολη ουρας εκτυπωσης
      </button>
      <button onclick="document.getElementById('custom-confirm-modal').remove()"
        style="width:100%;padding:12px;border-radius:10px;border:1px solid #ddd;background:#f5f5f5;color:#444;font-size:13px;cursor:pointer">
        Ακυρωση
      </button>
    </div>`;
  document.body.appendChild(m);
...
// ============================================================
// MIGRATION — μετονομασια/διαχωρισμος κατηγοριων στο localStorage
// Τρεχει μια φορα, ασφαλης αν ξανατρεξει
// ============================================================
function runCategoryMigrations() {
  if (!window.PRODUCTS) return;

  // Χαρτης: παλιο ονομα → νεο ονομα
  const RENAMES = {
    'ΑΡΤΟΠΟΙΗΜΑΤΑ / ΨΩΜΙΑ':           'Μπεργκερ/Μπαγκετες/Τοστ/Σαντουιτς',
    'ΚΡΟΚΕΤΕΣ / ΠΑΝΑΡΙΣΜΕΝΑ':         'ΚΡΟΚΕΤΕΣ / ΠΑΝΑΡΙΣΜΕΝΑ', // παραμενει ιδιο
    'ΛΑΧΑΝΙΚΑ / ΚΑΤΕΨΥΓΜΕΝΑ':         'ΛΑΧΑΝΙΚΑ / ΦΡΟΥΤΑ',
    // Η παλια ενιαια κατηγορια (αν υπαρχει απο προηγουμενο migration)
    'ΣΦΟΛΙΑΤΟΕΙΔΗ ΠΙΝΣΑ / ΠΙΤΣΑ / ΚΡΟΚΕΤΕΣ / ΠΑΝΑΡΙΣΜΕΝΑ': null, // θα διαχωριστει παρακατω
  };

  // Προϊοντα που ανηκουν στη ΣΦΟΛΙΑΤΟΕΙΔΗ / ΠΙΝΣΕΣ / ΠΙΤΣΕΣ (βαση supplier)
  const SFOLIATA_SUPPLIERS = new Set([
    'Μουρατιδης','Αραμπατζης','Κασσανδρα','Αρτομα','Λαμπρου','ΠιτΠαλε'
  ]);

  // 1. Μετονομασια απλων κατηγοριων στο PRODUCTS
  Object.entries(RENAMES).forEach(([oldName, newName]) => {
    if (!window.PRODUCTS[oldName]) return;
    if (newName && newName !== oldName) {
      // Συγχωνευση αν υπαρχει ηδη η νεα κατηγορια
      if (!window.PRODUCTS[newName]) window.PRODUCTS[newName] = [];
      // Προσθηκη μονο οσων δεν υπαρχουν ηδη (βαση id)
      const existingIds = new Set(window.PRODUCTS[newName].map(p => p.id));
      window.PRODUCTS[oldName].forEach(p => {
        if (!existingIds.has(p.id)) window.PRODUCTS[newName].push(p);
      });
      delete window.PRODUCTS[oldName];
    } else if (!newName) {
      // Η ενιαια κατηγορια → διαχωρισμος σε 2
      const prods = window.PRODUCTS[oldName] || [];
      if (!window.PRODUCTS['ΣΦΟΛΙΑΤΟΕΙΔΗ / ΠΙΝΣΕΣ / ΠΙΤΣΕΣ']) window.PRODUCTS['ΣΦΟΛΙΑΤΟΕΙΔΗ / ΠΙΝΣΕΣ / ΠΙΤΣΕΣ'] = [];
      if (!window.PRODUCTS['ΚΡΟΚΕΤΕΣ / ΠΑΝΑΡΙΣΜΕΝΑ']) window.PRODUCTS['ΚΡΟΚΕΤΕΣ / ΠΑΝΑΡΙΣΜΕΝΑ'] = [];
      const sfIds = new Set(window.PRODUCTS['ΣΦΟΛΙΑΤΟΕΙΔΗ / ΠΙΝΣΕΣ / ΠΙΤΣΕΣ'].map(p => p.id));
      const krIds = new Set(window.PRODUCTS['ΚΡΟΚΕΤΕΣ / ΠΑΝΑΡΙΣΜΕΝΑ'].map(p => p.id));
      prods.forEach(p => {
        if (SFOLIATA_SUPPLIERS.has(p.supplier)) {
          if (!sfIds.has(p.id)) window.PRODUCTS['ΣΦΟΛΙΑΤΟΕΙΔΗ / ΠΙΝΣΕΣ / ΠΙΤΣΕΣ'].push(p);
        } else {
          if (!krIds.has(p.id)) window.PRODUCTS['ΚΡΟΚΕΤΕΣ / ΠΑΝΑΡΙΣΜΕΝΑ'].push(p);
        }
      });
      delete window.PRODUCTS[oldName];
    }
  ...
  // 2. Ενημερωση ιστορικου πελατων — αντικατασταση παλιων κατηγοριων
  const CLIENT_RENAMES = {
    'ΑΡΤΟΠΟΙΗΜΑΤΑ / ΨΩΜΙΑ': 'Μπεργκερ/Μπαγκετες/Τοστ/Σαντουιτς',
    'ΛΑΧΑΝΙΚΑ / ΚΑΤΕΨΥΓΜΕΝΑ': 'ΛΑΧΑΝΙΚΑ / ΦΡΟΥΤΑ',
    'ΣΦΟΛΙΑΤΟΕΙΔΗ ΠΙΝΣΑ / ΠΙΤΣΑ / ΚΡΟΚΕΤΕΣ / ΠΑΝΑΡΙΣΜΕΝΑ': null, // διαχωρισμος βαση id
  };

  window.CLIENTS.forEach(c => {
    if (!c.history) return;
    Object.entries(CLIENT_RENAMES).forEach(([oldCat, newCat]) => {
      if (!c.history[oldCat]) return;
      if (newCat) {
        if (!c.history[newCat]) c.history[newCat] = {};
        Object.assign(c.history[newCat], c.history[oldCat]);
        delete c.history[oldCat];
      } else {
        // Διαχωρισμος: κοιτα τον supplier του καθε προιοντος
        if (!c.history['ΣΦΟΛΙΑΤΟΕΙΔΗ / ΠΙΝΣΕΣ / ΠΙΤΣΕΣ']) c.history['ΣΦΟΛΙΑΤΟΕΙΔΗ / ΠΙΝΣΕΣ / ΠΙΤΣΕΣ'] = {};
        if (!c.history['ΚΡΟΚΕΤΕΣ / ΠΑΝΑΡΙΣΜΕΝΑ']) c.history['ΚΡΟΚΕΤΕΣ / ΠΑΝΑΡΙΣΜΕΝΑ'] = {};
        Object.entries(c.history[oldCat]).forEach(([pid, qty]) => {
          const p = getProd(pid);
          const target = (p && SFOLIATA_SUPPLIERS.has(p.supplier))
            ? 'ΣΦΟΛΙΑΤΟΕΙΔΗ / ΠΙΝΣΕΣ / ΠΙΤΣΕΣ'
            : 'ΚΡΟΚΕΤΕΣ / ΠΑΝΑΡΙΣΜΕΝΑ';
          c.history[target][pid] = qty;
        });
        delete c.history[oldCat];
      }
    });
  ...
  // 3. Μετακινηση μεμονωμενων προιοντων σε λαθος κατηγορια
  // sl29 = Κρουτον → απο Μπεργκερ/Μπαγκετες ή ΑΡΤΟΠΟΙΗΜΑΤΑ στην Αποθηκη
  const PROD_MOVES = [
    { id: 'sl29', fromCat: 'Μπεργκερ/Μπαγκετες/Τοστ/Σαντουιτς', toCat: 'ΑΠΟΘΗΚΗ / ΞΗΡΟ ΚΟΜΜΑΤΙ' },
    { id: 'sl29', fromCat: 'ΑΡΤΟΠΟΙΗΜΑΤΑ / ΨΩΜΙΑ',               toCat: 'ΑΠΟΘΗΚΗ / ΞΗΡΟ ΚΟΜΜΑΤΙ' },
  ];
  PROD_MOVES.forEach(({ id, fromCat, toCat }) => {
    // Μετακινηση στο PRODUCTS
    if (window.PRODUCTS[fromCat]) {
      const idx = window.PRODUCTS[fromCat].findIndex(p => p.id === id);
      if (idx !== -1) {
        const [prod] = window.PRODUCTS[fromCat].splice(idx, 1);
        if (!window.PRODUCTS[toCat]) window.PRODUCTS[toCat] = [];
        if (!window.PRODUCTS[toCat].find(p => p.id === id)) {
          window.PRODUCTS[toCat].push(prod);
        }
      }
    }
    // Μετακινηση στο ιστορικο πελατων
    window.CLIENTS.forEach(c => {
      if (!c.history || !c.history[fromCat] || c.history[fromCat][id] === undefined) return;
      const qty = c.history[fromCat][id];
      delete c.history[fromCat][id];
      if (Object.keys(c.history[fromCat]).length === 0) delete c.history[fromCat];
      if (!c.history[toCat]) c.history[toCat] = {};
      c.history[toCat][id] = qty;
    });
  });
...
document.addEventListener('DOMContentLoaded', () => {
  loadData();
  runCategoryMigrations(); // migration κατηγοριων
...
// ============================================================
// ΠΡΟΓΡΑΜΜΑ ΕΠΙΣΚΕΨΕΩΝ / ΠΑΡΑΓΓΕΛΙΩΝ
// ============================================================

const DAYS_EL = ['Κυρ', 'Δευ', 'Τρι', 'Τετ', 'Πεμ', 'Παρ', 'Σαβ'];
const DAYS_FULL = ['Κυριακη', 'Δευτερα', 'Τριτη', 'Τεταρτη', 'Πεμπτη', 'Παρασκευη', 'Σαββατο'];
const FREQ_OPTS = [
  { val: 1, label: 'Καθε εβδομαδα' },
  { val: 2, label: 'Καθε 2 εβδομαδες' },
  { val: 3, label: 'Καθε 3 εβδομαδες' },
  { val: 4, label: 'Καθε μηνα' },
];

let scheduleWeekOffset = 0; // 0=τρεχουσα, -1=προηγουμενη, 1=επομενη
...
// Επιστρέφει τις μέρες που έχει επίσκεψη ο πελάτης για μια δεδομένη εβδομάδα
function getClientDaysForWeek(c, weekStart) {
  const sched = c.schedule;
  if (!sched || !sched.days || !sched.days.length) return [];
  const freq = sched.freq || 1;

  // Override check
  if (sched.override && sched.override.from && sched.override.to) {
    const from = new Date(sched.override.from);
    const to   = new Date(sched.override.to);
    const wEnd = new Date(weekStart); wEnd.setDate(weekStart.getDate() + 6);
    // Αν η εβδομάδα εμπίπτει στο override range
    if (weekStart <= to && wEnd >= from) {
      return (sched.override.days || []).map(Number);
    }
  }

  // Frequency check — μετράμε εβδομάδες από epoch
  const epochMon = new Date(2024, 0, 1); // αρχή μέτρησης
  const diffMs = weekStart - epochMon;
  const diffWeeks = Math.floor(diffMs / (7 * 24 * 60 * 60 * 1000));
  if (freq > 1 && diffWeeks % freq !== 0) return [];

  return sched.days.map(Number);
}

// Ελέγχει αν πελάτης έχει "Ολοκληρωθεί" σήμερα
function isClientDoneToday(c) {
  const todayKey = dateKey(new Date());
  return !!(c.schedule && c.schedule.doneOn && c.schedule.doneOn[todayKey]);
}

// Ελέγχει αν πελάτης έχει επίσκεψη σήμερα
function isClientScheduledToday(c) {
  const today = new Date();
  const dow = today.getDay(); // 0=Sun
  const weekStart = getWeekStart(0);
  const days = getClientDaysForWeek(c, weekStart);
...
function renderScheduleScreen() {
  ...
  // Label εβδομάδας
  const fmt = d => d.toLocaleDateString('el-GR', { day: 'numeric', month: 'numeric' });
  const lbl = scheduleWeekOffset === 0 ? 'Τρεχουσα εβδομαδα' :
              scheduleWeekOffset === -1 ? 'Προηγουμενη εβδομαδα' :
              scheduleWeekOffset === 1  ? 'Επομενη εβδομαδα' :
              `${fmt(weekStart)} – ${fmt(weekEnd)}`;
  const el = document.getElementById('schedule-week-label');
  if (el) el.textContent = `${fmt(weekStart)} – ${fmt(weekEnd)} · ${lbl}`;
  ...
  let html = '';
  workDays.forEach(dow => {
    const dayDate = new Date(weekStart);
    dayDate.setDate(weekStart.getDate() + (dow === 0 ? 6 : dow - 1));
    const isToday = dayDate.getTime() === today.getTime();
    const dayKey = dateKey(dayDate);

    // Πελάτες αυτής της μέρας
    const clients = window.CLIENTS.filter(c => {
      const days = getClientDaysForWeek(c, weekStart);
      return days.includes(dow);
    });

    if (!clients.length && !isToday) return; // παράλειψη κενών μερών (εκτός σήμερα)

    // Ομαδοποίηση ανά δρομολόγιο
    const byRoute = {};
    clients.forEach(c => {
      const rk = c.route || 'other';
      if (!byRoute[rk]) byRoute[rk] = [];
      byRoute[rk].push(c);
    });

    const doneCount = clients.filter(c => {
      return c.schedule && c.schedule.doneOn && c.schedule.doneOn[dayKey];
    }).length;

    const todayStyle = isToday
      ? 'border:1.5px solid #1D9E75;background:#E1F5EE'
      : 'border:0.5px solid #e0e0e0;background:#fff';

    html += `<div style="${todayStyle};border-radius:12px;padding:12px;margin-bottom:10px">`;
    html += `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">`;
    html += `<div style="font-size:14px;font-weight:700;color:${isToday ? '#0F6E56' : '#1a1a1a'}">`;
    html += `${DAYS_FULL[dow]} ${dayDate.toLocaleDateString('el-GR', { day:'numeric', month:'numeric' })}`;
    if (isToday) html += ` <span style="font-size:11px;font-weight:500;color:#1D9E75">· σημερα</span>`;
    html += `</div>`;
    if (clients.length) {
      html += `<span style="font-size:12px;color:${doneCount===clients.length?'#1D9E75':'#888'};font-weight:600">`;
      html += `${doneCount}/${clients.length} ✓</span>`;
    }
    html += `</div>`;

    if (!clients.length) {
      html += `<div style="font-size:12px;color:#aaa">Κανενας πελατης</div>`;
    } else {
      Object.entries(byRoute).forEach(([rk, rClients]) => {
        const routeInfo = ROUTES.find(r => r.key === rk);
        html += `<div style="font-size:11px;font-weight:600;color:#888;text-transform:uppercase;letter-spacing:0.4px;margin:4px 0 2px">`;
        html += `<i class="ti ti-route" style="font-size:10px" aria-hidden="true"></i> ${routeInfo ? routeInfo.label : rk}</div>`;
        rClients.forEach(c => {
          const done = c.schedule && c.schedule.doneOn && c.schedule.doneOn[dayKey];
          const timeStr = c.schedule && c.schedule.timeFrom ? ` · ${c.schedule.timeFrom}${c.schedule.timeTo ? '–' + c.schedule.timeTo : ''}` : '';
          const rowBg    = done ? '#E1F5EE' : '#FFFBEB';
          const rowBord  = done ? '#5DCAA5' : '#FCD34D';
          const nameCol  = done ? '#0F6E56' : '#633806';
          html += `<div style="display:flex;align-items:center;gap:8px;padding:6px 8px;margin:3px 0;border-radius:8px;background:${rowBg};border:1px solid ${rowBord}">`;
          html += `<div style="flex:1;font-size:13px;font-weight:600;color:${nameCol}">`;
          html += `${c.name}${timeStr}</div>`;
          if (done) {
            html += `<span style="font-size:11px;font-weight:600;color:#0F6E56;white-space:nowrap"><i class="ti ti-check" style="font-size:10px" aria-hidden="true"></i> Ολοκληρωθηκε</span>`;
          }
          html += `</div>`;
        });
      });
  ...
  if (!html) html = `<div style="text-align:center;padding:40px 0;color:#aaa;font-size:14px">Κανενα προγραμμα αυτη την εβδομαδα</div>`;
...
function buildScheduleHTML(c) {
  const sched = c.schedule || {};
  const selDays = sched.days || [];
  const freq = sched.freq || 1;
  const timeFrom = sched.timeFrom || '';
  const timeTo   = sched.timeTo   || '';
  const ovFrom   = sched.override ? (sched.override.from || '') : '';
  const ovTo     = sched.override ? (sched.override.to   || '') : '';
  const ovDays   = sched.override ? (sched.override.days || []) : [];

  const dayBtns = [1,2,3,4,5,6].map(d =>
    `<button type="button" onclick="toggleSchedDay(${d},this)"
      style="padding:6px 10px;border-radius:20px;border:1px solid ${selDays.includes(d)?'#1D9E75':'#ddd'};
      background:${selDays.includes(d)?'#E1F5EE':'#f9f9f9'};color:${selDays.includes(d)?'#0F6E56':'#666'};
      font-size:13px;font-weight:${selDays.includes(d)?'700':'400'};cursor:pointer"
      data-day="${d}" data-active="${selDays.includes(d)?'1':'0'}">${DAYS_EL[d]}</button>`
  ).join('');

  const freqOpts = FREQ_OPTS.map(f =>
    `<option value="${f.val}"${f.val===freq?' selected':''}>${f.label}</option>`
  ).join('');

  const ovDayBtns = [1,2,3,4,5,6].map(d =>
    `<button type="button" onclick="toggleOvDay(${d},this)"
      style="padding:5px 9px;border-radius:20px;border:1px solid ${ovDays.includes(d)?'#EF9F27':'#ddd'};
      background:${ovDays.includes(d)?'#FAEEDA':'#f9f9f9'};color:${ovDays.includes(d)?'#633806':'#666'};
      font-size:12px;cursor:pointer"
      data-day="${d}" data-active="${ovDays.includes(d)?'1':'0'}">${DAYS_EL[d]}</button>`
  ).join('');

  return `
    <div style="margin-top:18px;border-top:0.5px solid #eee;padding-top:14px">
      <div style="font-size:14px;font-weight:700;color:#1a1a1a;margin-bottom:12px">
        <i class="ti ti-calendar-week" style="color:var(--green)" aria-hidden="true"></i> Προγραμμα επισκεψης
      </div>

      <div class="field-label">Μερες επισκεψης</div>
      <div id="sched-days" style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:12px">${dayBtns}</div>

      <div class="field-label">Συχνοτητα</div>
      <select class="field-input" id="sched-freq" style="margin-bottom:12px">${freqOpts}</select>

      <div style="display:flex;gap:8px;margin-bottom:12px">
        <div style="flex:1">
          <div class="field-label">Ωρα απο</div>
          <input type="text" inputmode="numeric" class="field-input" id="sched-time-from" value="${timeFrom}" placeholder="π.χ. 09:00" maxlength="5" oninput="fmtTime(this)">
        </div>
        <div style="flex:1">
          <div class="field-label">Ωρα εως</div>
          <input type="text" inputmode="numeric" class="field-input" id="sched-time-to" value="${timeTo}" placeholder="π.χ. 17:00" maxlength="5" oninput="fmtTime(this)">
        </div>
      </div>

      <div style="background:#FFFBEB;border:0.5px solid #FCD34D;border-radius:10px;padding:12px;margin-bottom:8px">
        <div style="font-size:12px;font-weight:700;color:#92400E;margin-bottom:8px">
          <i class="ti ti-calendar-event" aria-hidden="true"></i> Προσωρινη αλλαγη (προαιρετικο)
        </div>
        <div style="display:flex;gap:8px;margin-bottom:8px">
          <div style="flex:1">
            <div class="field-label" style="font-size:11px">Απο</div>
            <input type="date" class="field-input" id="sched-ov-from" value="${ovFrom}" style="font-size:13px;padding:7px">
          </div>
          <div style="flex:1">
            <div class="field-label" style="font-size:11px">Εως</div>
            <input type="date" class="field-input" id="sched-ov-to" value="${ovTo}" style="font-size:13px;padding:7px">
          </div>
        </div>
        <div class="field-label" style="font-size:11px">Μερες για αυτο το διαστημα</div>
        <div id="sched-ov-days" style="display:flex;gap:5px;flex-wrap:wrap">${ovDayBtns}</div>
        ${ovFrom ? `<button type="button" onclick="clearOverride()" style="margin-top:8px;border:none;background:transparent;color:#DC2626;font-size:12px;cursor:pointer"><i class="ti ti-x" style="font-size:11px"></i> Καθαρισμος αλλαγης</button>` : ''}
      </div>
    </div>`;
}

function toggleSchedDay(d, btn) {
  const active = btn.getAttribute('data-active') === '1';
  const newActive = !active;
  btn.setAttribute('data-active', newActive ? '1' : '0');
  btn.style.background    = newActive ? '#E1F5EE' : '#f9f9f9';
  btn.style.borderColor   = newActive ? '#1D9E75' : '#ddd';
  btn.style.color         = newActive ? '#0F6E56' : '#666';
  btn.style.fontWeight    = newActive ? '700' : '400';
}

function toggleOvDay(d, btn) {
  const active = btn.getAttribute('data-active') === '1';
  const newActive = !active;
  btn.setAttribute('data-active', newActive ? '1' : '0');
  btn.style.background  = newActive ? '#FAEEDA' : '#f9f9f9';
  btn.style.borderColor = newActive ? '#EF9F27' : '#ddd';
  btn.style.color       = newActive ? '#633806' : '#666';
}

function clearOverride() {
  const f = document.getElementById('sched-ov-from');
  const t = document.getElementById('sched-ov-to');
  if (f) f.value = '';
  if (t) t.value = '';
  document.querySelectorAll('#sched-ov-days button').forEach(b => {
    b.setAttribute('data-active','0');
    b.style.background='#f9f9f9'; b.style.borderColor='#ddd'; b.style.color='#666';
  });
}
...
function readScheduleFromModal() {
  ...
  document.querySelectorAll('#sched-days button').forEach(b => {
    if (b.getAttribute('data-active')==='1') days.push(Number(b.getAttribute('data-day')));
  });
  const freq      = parseInt(document.getElementById('sched-freq')?.value || '1');
  const timeFrom  = document.getElementById('sched-time-from')?.value || '';
  const timeTo    = document.getElementById('sched-time-to')?.value   || '';
  const ovFrom    = document.getElementById('sched-ov-from')?.value   || '';
  const ovTo      = document.getElementById('sched-ov-to')?.value     || '';
  const ovDays    = [];
  document.querySelectorAll('#sched-ov-days button').forEach(b => {
    if (b.getAttribute('data-active')==='1') ovDays.push(Number(b.getAttribute('data-day')));
  });
  const schedule = { days, freq, timeFrom, timeTo };
  if (ovFrom && ovTo) schedule.override = { from: ovFrom, to: ovTo, days: ovDays };
  return schedule;
}

// ---- Σήμανση "Ολοκληρωθηκε" μετά εκτύπωση ----

function markScheduleDoneFromPrint() {
  const q = getPrintQueue();
  const todayKey = dateKey(new Date());
  q.forEach(order => {
    const c = window.CLIENTS.find(cl => cl.id === order.clientId);
    if (!c || !c.schedule) return;
    if (!isClientScheduledToday(c)) return;
    if (!c.schedule.doneOn) c.schedule.doneOn = {};
    c.schedule.doneOn[todayKey] = true;
  });
  autoSave();
  filterClients('');
}
