...
function goSummary() {
  ...
  if (hasQtyNotChecked.length) {
    const names = hasQtyNotChecked.map(([pid]) => getProd(pid)?.name || pid).slice(0,3).join('\n• ');
    const proceed = confirm(`⚠️ Προσοχη! ${hasQtyNotChecked.length} προιον(τα) εχουν ποσοτητα αλλα ΔΕΝ ειναι τσεκαρισμενα:\n• ${names}${hasQtyNotChecked.length > 3 ? '\n...' : ''}\n\nΣυνεχεια χωρις αυτα;`);
    if (!proceed) return;
  }
...
function openPrintPreview() {
  ...
  let cols = q.map(order => {
    ...
    Object.entries(order.zones).forEach(([zk, zone]) => {
      ...
      const zoneStyle = isSyn ? 'style="background:#c0c0c0;color:#000;"'
                      : isApo ? 'style="background:url(\'data:image/svg+xml,<svg xmlns=\\\'http://www.w3.org/2000/svg\\\' width=\\\'4\\\' height=\\\'4\\\'><line x1=\\\'0\\\' y1=\\\'4\\\' x2=\\\'4\\\' y2=\\\'0\\\' stroke=\\\'%23999\\\' stroke-width=\\\'0.8\\\'/></svg>\') repeat;color:#000;"'
                      : '';
      const itemStyle = isSyn ? 'style="background:#d8d8d8;"'
                      : isApo ? 'style="background:url(\'data:image/svg+xml,<svg xmlns=\\\'http://www.w3.org/2000/svg\\\' width=\\\'4\\\' height=\\\'4\\\'><line x1=\\\'0\\\' y1=\\\'4\\\' x2=\\\'4\\\' y2=\\\'0\\\' stroke=\\\'%23bbb\\\' stroke-width=\\\'0.6\\\'/></svg>\') repeat;"'
                      : '';
  ...
  window._printHTML = `<!DOCTYPE html><html lang="el"><head>
    ...
    <style>
      * { margin:0; padding:0; box-sizing:border-box; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
      ...
      .print-cat { font-size:9px; font-weight:600; color:#000; margin:2px 0 1px 0; border-bottom:0.5px solid #ccc; display:flex; justify-content:space-between; gap:4px; padding:0 2px; }
      .print-cat-total { font-weight:700; color:#000; white-space:nowrap; }
      .print-item { font-size:10px; padding:1px 0 1px 6px; display:flex; gap:4px; }
