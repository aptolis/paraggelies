    ...
    byRoute[r.key].forEach(c => {
      ...
      html += `<div class="client-card" id="card-${c.id}" onclick="selectClient('${c.id}')"${inQueue ? ' style="border:1.5px solid var(--green);background:var(--green-light)"' : ''}>
        ...
        <div style="flex:1;min-width:0">
          ...
          ${c.lastOrderAt ? `<div class="client-meta" style="color:var(--color-text-tertiary);font-size:10px"><i class="ti ti-clock" style="font-size:10px"></i> Τελ. παραγγελια ${new Date(c.lastOrderAt).toLocaleDateString('el-GR',{weekday:'short',day:'2-digit',month:'2-digit',year:'2-digit'})} ${new Date(c.lastOrderAt).toLocaleTimeString('el-GR',{hour:'2-digit',minute:'2-digit'})}</div>` : ''}
        </div>
