...
function emailBackup() {
  ...
  const body     = encodeURIComponent(`Backup εφαρμογής Παραγγελίες\nΗμερομηνία: ${new Date().toLocaleString('el-GR')}\n\n(Επισύναψε το αρχείο ${filename} από τα Downloads)`);
  const subject  = encodeURIComponent(`Backup Παραγγελίες ${new Date().toLocaleDateString('el-GR')}`);

  // Πρώτα κατεβάζει το αρχείο, μετά ανοίγει το email
  ...
  // Μετά από λίγο ανοίγει το email app με προσυμπληρωμένα στοιχεία
  setTimeout(() => {
    window.location.href = `mailto:tolis.diaf@gmail.com?subject=${subject}&body=${body}`;
    showToast('Άνοιξε το email και επίσυναψε το αρχείο!', 'success');
  }, 800);
}
